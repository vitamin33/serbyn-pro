...
slug: one-sided-contracts-break-agent-pipelines
...
```mermaid
flowchart TD
 A[Canonical Outcome Schema] -->|derives vocabulary| B[Seed Generator]
 A -->|defines accepted set| C[Gate Validator]
 B -->|injects exact strings into| D[Dispatch Seed]
 D -->|instructs| E[Agent]
 E -->|emits outcome value| F[Gate]
 F -->|validates against| C
 G[Cross-Check Test] -->|asserts seed vocab == gate vocab| A
```

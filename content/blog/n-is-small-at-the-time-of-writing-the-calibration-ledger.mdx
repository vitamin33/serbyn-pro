---
title: "The Calibration Ledger: 52 Runs, 94% Pass, n Is Small"
slug: n-is-small-at-the-time-of-writing-the-calibration-ledger
description: "52 runs, ~94% autonomous pass, and why that number is honest evidence — not a reliability proof. A field report on agent evals at solo scale."
date: '2026-06-22'
keywords:
...
# The Calibration Ledger: 52 Runs, 94% Pass, n Is Small

The calibration ledger holds ~52 runs at ~94% autonomous pass — and the most important thing I can tell you about that number is what it does not prove.
...
I will keep porting it and keep running it. The ledger will accumulate. At some threshold of runs across multiple repos, the generalization claim becomes defensible. That threshold is not ~52.
...
Despite all of the above, the ledger earns its existence several times over, even at ~52 runs.
...
This separation means I can re-judge historical runs when I upgrade a judgment function. If I deploy a calibrated LLM-as-judge tomorrow, I can run it against the existing ~52 run artifacts and see how its verdicts compare to the original verdicts. That is how I will eventually build the calibration curve for the judge itself — by accumulating disagreements between old and new judgment functions on the same underlying runs.
...
~52 runs. ~94% autonomous pass. An append-only ledger. No isolation sandbox. Unit-tier evals without trajectory scoring. One successful smoke test on a second repo. A bypass flag that is one mistake from an incident. A judge calibration problem I have named but not solved.
...
Keep the ledger before you need it. The calibration instrument has to predate the reliability claim — not follow it. If you build the ledger after you have already been telling people the agent is reliable, you have reversed the epistemic order and the ledger is now rationalization rather than evidence. Start append-only, name every gap explicitly, and let the number be whatever it actually is. ~52 runs at ~94% pass is an honest number. An honest number is the only kind worth keeping.
